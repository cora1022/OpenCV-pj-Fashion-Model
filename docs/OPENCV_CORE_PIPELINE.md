...
## 10. Qdrant 데이터 구축 스크립트에서도 OpenCV 사용
...
이 스크립트는 프로젝트를 만들기 전에 Qdrant 기준 데이터를 구축하기 위해 사용한다. 네이버 쇼핑 API로 상품 이미지를 수집하고, 저장 전에 의류 영역을 crop한 뒤 FashionCLIP 벡터로 변환한다. 따라서 Qdrant에 저장되는 기준 이미지들도 OpenCV 전처리를 거친 의류 중심 이미지가 된다.

데이터 구축 단계의 흐름은 다음과 같다.

```text
1. 네이버 쇼핑 API에서 패션 상품 목록을 가져온다.
2. 상품 이미지 URL로 원본 이미지를 다운로드한다.
3. 이미지를 PIL RGB 형식으로 통일한다.
4. numpy array로 바꾼다.
5. OpenCV 처리를 위해 RGB를 BGR로 변환한다.
6. OpenCV HOG detector로 사람/상반신 후보 영역을 찾는다.
7. 후보 영역을 GUI의 빨간 crop box로 표시한다.
8. 사용자가 의류 영역에 맞게 box를 보정하거나 직접 다시 선택한다.
9. crop된 이미지를 FashionCLIP에 넣어 벡터화한다.
10. Qdrant에 vector와 상품 payload를 upsert한다.
```

이 단계가 중요한 이유는 Qdrant가 검색 기준 데이터베이스이기 때문이다. Qdrant에 원본 이미지 전체가 들어가면 배경, 모델 얼굴, 포즈, 여백, 쇼핑몰 워터마크 같은 요소가 벡터에 섞일 수 있다. 반대로 Qdrant 구축 시점부터 OpenCV로 의류 관심 영역을 잡아 crop하면, 이후 사용자가 업로드한 이미지와 비교할 때 의류 특징끼리 더 일관되게 비교할 수 있다.

`naver_crop_to_qdrant_fashionclip.py`에는 OpenCV 자동 후보 영역 기능도 들어 있다.
...
`A`를 누르면 OpenCV HOG detector가 후보 영역을 제안한다. 이 후보를 그대로 저장할 수도 있고, 사람이 직접 박스를 조정해 더 정확한 의류 영역을 선택할 수도 있다. 이렇게 저장된 데이터에는 `crop_used`, `crop_method`, `saved_image_path`, `embedding_model`, `collection_name` 같은 payload가 함께 기록된다.
