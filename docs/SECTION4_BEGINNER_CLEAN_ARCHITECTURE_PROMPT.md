...
7. OpenCV HOG detector로 사람/상반신 후보 영역을 찾는다.
8. GUI에 OpenCV 추천 crop box를 표시한다.
9. 사용자가 의류 영역에 맞게 crop box를 보정한다.
10. crop된 이미지를 FashionCLIP으로 벡터화한다.
11. Qdrant 컬렉션에 벡터와 상품 payload를 저장한다.
...
- payload에는 crop_used, crop_method, saved_image_path를 남겨 Qdrant 구축 과정에서 OpenCV/수동 crop이 사용되었는지 확인할 수 있게 한다.
```

데이터 구축 스크립트도 서비스와 같은 관점으로 작성한다. API 서버와 직접 연결되는 코드는 아니지만, 이미지 전처리, 임베딩, 벡터 DB 저장이라는 핵심 파이프라인을 보여주는 중요한 코드이다. 특히 Qdrant는 검색 기준 데이터가 들어가는 곳이므로, 이 단계에서 OpenCV로 의류 영역을 먼저 정리해두어야 이후 업로드 이미지와 비교할 때 기준이 맞는다.
